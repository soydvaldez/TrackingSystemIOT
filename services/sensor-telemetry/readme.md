...
#### Construye el servicio
  ...
  **Clona el proyecto, configura y construye el proyecto con Gradle:**
  ``` sh
  git clone git@192.168.1.174:insighttechio/sensor-telemetry-service.git
  cd sensor-telemetry/
...
##### Utilizando Docker
   ...
   ```sh
    docker build \
     -t localhost:5000/sensor-telemetry-service:1.0.0 \
     -t localhost:5000/sensor-telemetry-service:latest .
   ```

   - Subir la imagen a un Docker Registry: **(Opcional)**
   ```sh
    docker push localhost:5000/sensor-telemetry-service:1.0.0
    docker push localhost:5000/sensor-telemetry-service:latest
   ```

   - Levantar el servicio en un contenedor docker:
   ```sh
   docker run --rm --name sensor-telemetry-container \
     -p 8000:8000 localhost:5000/sensor-telemetry-service:1.0.0
   ```
