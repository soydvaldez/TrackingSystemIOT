...
#### Construye el servicio
  ...
  **Clona el proyecto, configura y construye el proyecto con Gradle:**
  ``` sh
  git clone git@192.168.1.174:insighttechio/device-telemetry-service.git
  cd device-telemetry/
...
##### Utilizando Docker
   ...
   ```sh
    docker build \
     -t localhost:5000/device-telemetry-service:1.0.0 \
     -t localhost:5000/device-telemetry-service:latest .
   ```

   - Subir la imagen a un Docker Registry: **(Opcional)**
   ```sh
    docker push localhost:5000/device-telemetry-service:1.0.0
    docker push localhost:5000/device-telemetry-service:latest
   ```

   - Levantar el servicio en un contenedor docker:
   ```sh
   docker run --rm --name device-telemetry-container \
     -p 8000:8000 localhost:5000/device-telemetry-service:1.0.0
   ```
