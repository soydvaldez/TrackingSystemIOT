...
echo
echo "==================================================================================="
echo "Bienvenido al script de construccion y despliegue de la App con docker engine." | toUpperCase
echo
...
# Imprime las variables del contexto de ejecucion del script:
...
# Construye el ejecutable JAR del proyecto con un perfil especificado: "default, dev o prod"
# Ejecuta las pruebas de desarrollo:
function build_jar() {
    ./gradlew clean build -Pprofile=${SPRING_PROFILES_ACTIVE}
    # ./gradlew clean build
}
...
# Hace push de la imagen Docker generada hacia el "Docker Registry" para guardarla:
...
# Lista las imagenes desde el "Docker Registry"
function list_images() {
    echo "localhost:5000/"${IMAGE_NAME}":${TAGS[0]}"
    echo "localhost:5000/"${IMAGE_NAME}":${TAGS[1]}"
}

# Punto principal de aranque del script (agrupa las principales funciones)
